...
- **Known defect — not fixed, not automated (team decision):** submitting registration with `firstName`, `lastName`, `email`, or `password` missing (`=== undefined`) crashes the Node process instead of returning `400` — a synchronous `throw` inside an `async` controller with no `try/catch` and no global Express error handler (`backend/app/controllers/user.controller.js`). Reproduced empirically on 2026-09-14. The identical pattern exists in `recipeIngredient.controller.js#create` (also declared `async`) — same crash. `recipe.controller.js#create`, `recipeStep.controller.js#create`, and `ingredient.controller.js#create` use the same `throw`-based validation but are declared as plain (non-`async`) functions, so Express's default error handler catches the throw safely there instead — a real `400` status, but as an HTML stack-trace body rather than the app's usual `{ message }` JSON (see Feature 2 Edge Cases). All of this is out of scope for this feature per team decision — no code changes.
